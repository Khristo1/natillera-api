    ...
    def gestionar_prestamos(self):
        ...
        def registrar_pago():
            ...
            q = """SELECT saldo_pendiente, cuota_mensual, interes_mensual, 
                        cuotas_restantes, fecha_proximo_pago
            ...
            tk.Label(info_frame, text=f"Interés del período: ${interes_periodo:,.2f} ({interes_porcentaje}%)", font=("Arial", 10), fg="blue").pack(anchor=tk.W)
            ...
            ttk.Label(monto_frame, text="Monto a pagar ($):").pack(anchor=tk.W)
            ...
            def actualizar_desglose(event=None):
                try:
                    monto_pagado = float(entry_monto.get().replace(',', '')) if entry_monto.get() else 0
                    ...
                    if monto_pagado >= interes_periodo:
                        ...
                        if nuevo_saldo < 0:
                            nuevo_saldo = 0
                        # RECÁLCULO DEL NUEVO INTERÉS SOBRE EL NUEVO SALDO
                        nuevo_interes = nuevo_saldo * (interes_porcentaje / 100)
                        lbl_desglose.config(
                            text=f"✓ Desglose del pago:\n"
                                f"   Interés pagado: ${interes_periodo:,.2f}\n"
                                f"   Abono a capital: ${abono_capital:,.2f}\n"
                                f"   Nuevo saldo: ${nuevo_saldo:,.2f}\n"
                                f"   ▶ Próximo interés: ${nuevo_interes:,.2f}",
                            fg="green")
                    else:
                        lbl_desglose.config(
                            text=f"⚠️ El pago (${monto_pagado:,.2f}) no cubre el interés (${interes_periodo:,.2f})",
                            fg="red")
                except:
            ...
            def guardar_pago():
                try:
                    ...
                    if monto_pagado < interes_periodo:
                        messagebox.showwarning("Error", f"El pago debe cubrir al menos el interés (${interes_periodo:,.2f})")
                        return
                    ...
                    if nuevo_saldo > 0 and cuotas_restantes > 0:
                        nuevo_interes = nuevo_saldo * (interes_porcentaje / 100)
                        nueva_cuota = (nuevo_saldo / cuotas_restantes) + nuevo_interes
                    else:
                        nueva_cuota = 0
                    
                    nuevas_cuotas_restantes = cuotas_restantes if nuevo_saldo > 0 else 0
                    ...
                    """, (prestamo_actual_id, monto_pagado, fecha_pago, forma_pago,
                        interes_periodo, abono_capital, nuevo_saldo))
                    
                    # Actualizar préstamo con NUEVA CUOTA recalcilada
                    self.db.execute("""
                        UPDATE prestamos 
                    ...
                    messagebox.showinfo("Éxito", 
                        ...
                        f"📅 Próximo pago: {fecha_proxima.strftime('%Y-%m-%d')}")
